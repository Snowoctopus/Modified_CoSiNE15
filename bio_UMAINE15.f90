...
!                                                                      !
!    iNO3_      Nitrate                                                ! 
!    iNO2_      Nitrite !MGmodification!                               !  
!    iSiOH      Silicate                                               !
!    iNH4_      Ammonium                                               !
!    iAOO      Ammonia Oxidation Organisms  !MGmodification!           ! 
!    iNOO      Nitrite Oxidation Organisms  !MGmodification!           ! 
...
!  statevariables                                                      !
!  Further modified and developed by Moge Du adding 3 more             !
!  nitrification-related state variables (NO2 AOO NOO)                 !
!                                                                      !
!  Last: Moge Du 01/2019                                               !
!                                                                      !
...
!
      ...
      END IF
!
!MGmodification:change 15 variables to 18!
#ifdef PROFILE
...
!MGmodification: Add the NO2 related sourece and sink terms-Qsms18!
...
!
      ...
      idsink(5)=iChl1
      idsink(6)=iChl2
      idsink(7)=iAOO !MGmodification!
      idsink(8)=iNOO !MGmodification!
...
!
      ...
      Wbio(7)=0.0_r8                ! iAOO !MGmodification!
      Wbio(8)=0.0_r8                ! iNOO !MGmodification!
...
!            Bio_bak(i,k,iNO2_)=MIN(t(i,j,k,nstp,iNO2_),60.0_r8)
!            Bio(i,k,iNO2_)=Bio_bak(i,k,iNO2_)
...
!            Bio_bak(i,k,iAOO_)=MIN(t(i,j,k,nstp,iAOO_),60.0_r8)
!            Bio(i,k,iAOO_)=Bio_bak(i,k,iAOO_)
!            Bio_bak(i,k,iNOO_)=MIN(t(i,j,k,nstp,iNOO_),60.0_r8)
!            Bio(i,k,iNOO_)=Bio_bak(i,k,iNOO_)
...
!     &       pnh4s1*Bio(i,k,iNO2_)/akno2s1(ng)
!    uno3s1 = pnh4s1*Bio(i,k,iNO3_)/(akno3s1(ng)*cens1)
!    uno2s1 = pnh4s1*Bio(i,k,iNO2_)/(akno2s1(ng)*cens1)
...
!! growth
!    
!    grows1=min(uno3s1+unh4s1+uno2s1,upo4s1,uco2s1)*alts1
!
!! adjustments
!
!    cents1=grows1/(uno3s1+unh4s1+uno2s1+Minval)
!    uno3s1=cents1*uno3s1
!    uno2s1=cents1*uno2s1
...
!    pnh4s1=	1.0_r8/(1.0_r8+Bio(i,k,iNH4_)/aknh4s1(ng))
    ...
    uno2s1 = pnh4s1*Bio(i,k,iNO2_)/(akno2s1(ng)+Bio(i,k,iNO2_))
...
! growth
    ...
    grows1=min(uno3s1+unh4s1+uno2s1,upo4s1)*alts1
...
!    pnh4s2=	1.0_r8/(1.0_r8+Bio(i,k,iNH4_)/aknh4s2(ng))
    ...
    uno2s2 = pnh4s2*Bio(i,k,iNO2_)/(akno2s2(ng)+Bio(i,k,iNO2_))
...
! careful with this limitation term for diatoms
    ...
    grows2=min(uno3s2+unh4s2+uno2s2,usio4s2,upo4s2)*alts2
!    grows2=usio4s2*alts2

    cents2=grows2/(uno3s2+unh4s2+uno2s2+Minval)
    uno3s2=cents2*uno3s2
    uno2s2=cents2*uno2s2
    unh4s2=cents2*unh4s2
...
! uptake    
    ...
    gno2s1  = gmaxs1(ng)*uno2s1 !MGmodification! 
    gnh4s1  = gmaxs1(ng)*unh4s1
    gno3s2  = gmaxs2(ng)*uno3s2
    gno2s2  = gmaxs2(ng)*uno2s2 !MGmodification! 
    gnh4s2  = gmaxs2(ng)*unh4s2
    gsio4s2 = gmaxs2(ng)*usio4s2

!     -------------------------------------------------------
!     CALCULATING THE Ammonia Oxidation & Nitrite Oxidation !MGmodification!
!     -------------------------------------------------------
      unh4o1 = Bio(i,k,iNH4_)/(aknh4o1(ng)+Bio(i,k,iNH4_))
      gnh4o1 = gmaxo1(ng)*unh4o1
      uno2o2 = Bio(i,k,iNO2_)/(akno2o2(ng)+Bio(i,k,iNO2_))
      gno2o2 = gmaxo2(ng)*uno2o2
            
!     -------------------------------------------------------
!     CALCULATING THE NEW,REGENERATED,AND PRIMARY PRODUCTION !MGmodification!
!     -------------------------------------------------------
      nps1 =  (gno3s1+gno2s1) * Bio(i,k,iSphy)
      rps1 =  gnh4s1 * Bio(i,k,iSphy)
      nps2 =  (gno3s2+gno2s2) * Bio(i,k,iLphy) 
...
!
     ...
     &                    Chl2cs2_m(ng))
      ...
      npchl1=(gno3s1+gnh4s1+gno2s1)*(gno3s1+gnh4s1+gno2s1)      &
      &   *Chl2cs1_m(ng)      &
      &   *Bio(i,k,iChl1)/                             &
      &   (amaxs1(ng)*MAX(Chl2C_s1,Minval)*PAR(i,k)+Minval)     

      npchl2=(gno3s2+gnh4s2+gno2s2)*(gno3s2+gnh4s2+gno2s2)      &
      &   *Chl2cs2_m(ng)     &
      &   *Bio(i,k,iChl2)/                             &
...
!     -------------------------------------------------------
      ...
      morts3=bgamma8(ng)*Bio(i,k,iAOO) !MGmodification!
      morts4=bgamma8(ng)*Bio(i,k,iNOO) !MGmodification!
...
!another option
        II00=0.0095_r8
        nitrif1 = gnh4o1*Bio(i,k,iOxyg)/(Bio(i,k,iOxyg)+1.0)* &
       &       (1.0_r8-max(0.0_r8,(PAR(i,k)-II00)/   &
       &       (0.1_r8+PAR(i,k)-II00)))
        nitrif2 = gno2o2*Bio(i,k,iOxyg)/(Bio(i,k,iOxyg)+1.0)* &
       &       (1.0_r8-max(0.0_r8,(PAR(i,k)-II00)/   &
       &       (0.1_r8+PAR(i,k)-II00)))
...
!     -------------------------------------------------------
!     CALAULATING THE AOO & NOO !MGmodification!
!     -------------------------------------------------------

      rpo1 = nitrif1*Bio(i,k,iAOO)*0.0074
      npo2 = nitrif2*Bio(i,k,iNOO)*0.0025
...
!              &    /(akz1(ng)+Bio(i,k,iSphy)*Bio(i,k,iSphy))
!MGmodification!
      go1zz1 = beta1(ng)*Bio(i,k,iSzoo)*Bio(i,k,iAOO)                 &
              &    /(akz1(ng)+Bio(i,k,iAOO))
!MGmodification!
      go2zz1 = beta1(ng)*Bio(i,k,iSzoo)*Bio(i,k,iNOO)                 &
              &    /(akz1(ng)+Bio(i,k,iNOO))
...
! critical values to stop grazing
    ...
    !MGmodification!
    if(Bio(i,k,iAOO) .le. 0.005_r8)then
        go1zz1=0.0_r8
        gchl1zz1=0.0_r8    
    endif
    !MGmodification!
    if(Bio(i,k,iNOO) .le. 0.005_r8)then
        go2zz1=0.0_r8
        gchl1zz1=0.0_r8    
    endif
...
!    &    (1.0_r8-exp(-Bio(i,k,iOxyg)/30.0_r8))
      ...
      if(k .gt. 1)then  
        Qsms1 = - nps1 - nps2 + nitrif2*Bio(i,k,iNOO)*0.9975
        Qsms16 = -(gno2s1*Bio(i,k,iSphy))-(gno2s2*Bio(i,k,iLphy))   &
     &          -nitrif2*Bio(i,k,iNOO)+nitrif1*Bio(i,k,iAOO)*0.9926
        Qsms3 = - rps1 - rps2 + OXR*excrz1 + OXR*excrz2             &
     &          - rpo1*135+OXR*midd+OXR*(excrz1_2+excrz2_2)
      else
         Qsms1 = - nps1 - nps2 + nitrif2*Bio(i,k,iNOO)*0.9975
         Qsms16 = -(gno2s1*Bio(i,k,iSphy))-(gno2s2*Bio(i,k,iLphy))   &
     &          -nitrif2*Bio(i,k,iNOO)+nitrif1*Bio(i,k,iAOO)*0.9926
     ...
     & + 0.036_r8*SOC
        ...
        Qsms17 = + rpo1 - go1zz1 - morts3
        Qsms18 = + npo2 - go2zz1 - morts4  !MGmodification!   
     ...
     &             - gddzz2+ morts1 + morts2 + morts3 + morts4         &
     &             - OXR*midd+ remvz2                                  &
     &             + aggregs1 + aggregs2
...
!MGmodification!       
     ...
     &          -1.5*0.9926*nitrif1*Bio(i,k,iAOO)                        &
     &         - 0.5*0.9975*nitrif2*Bio(i,k,iNOO)                       &
...
! update values 
    ...
    Bio(i,k,iNO3_)=Bio(i,k,iNO3_)+dtdays*sms1
      Bio(i,k,iNO2_)=Bio(i,k,iNO2_)+dtdays*sms16
	...
	Bio(i,k,iPO4_)=Bio(i,k,iPO4_)+dtdays*sms10
      Bio(i,k,iAOO)=Bio(i,k,iAOO)+dtdays*sms17 !MGmodification!
      Bio(i,k,iNOO)=Bio(i,k,iNOO)+dtdays*sms18 !MGmodification!
...
!     &       (Bio_bak(i,k,iNO2_)-Bio(i,k,iNO2_))-                     
